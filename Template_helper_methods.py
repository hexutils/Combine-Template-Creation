...
def Unroll_2D_OnShell(fname):
        """Code written by Jeffrey Davis of happy hour cocktail fame to unroll a 2 dimensional histogram
        """
        histfile = ROOT.TFile.Open('fit_files/'+fname+'_2d.root', "READ")
        # print('fit_files/'+self.fname+'_2d_out.root')
        fout = ROOT.TFile('fit_files/'+fname+'_2d_out.root',"RECREATE")
        fout.cd()
        ...
        for keyname in ['ggH_0PM', 'ggH_0M', 'bkg_ggzz']:
            ...
            print('Dumped Histogram into fit_files/'+fname+'_2d_out.root')
            
# def create_datacards(output_folder, fname, verbose=False, only_1d = False, only_2d=False):
#     running_location = os.getcwd()
#     HexUtils_path = running_location[:running_location.find('HexUtils') + 8] #8 is the number of characters in "HexUtils"
